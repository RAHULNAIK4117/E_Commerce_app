...
const Add = ({ token }) => {
  ...
  const onsubmithandler = async (e) => {
    ...
    try {
      ...
      setSuccess("Product uploaded successfully! Refreshing for new entry...");
      ...
      console.error(error);
      setError(`Failed to upload product: ${error.response?.data?.message || error.message}`);
    }
  };
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100 p-4">
      <form onSubmit={onsubmithandler} className="bg-white p-6 rounded-lg shadow-xl w-full max-w-2xl animate-fadeIn">
        <h2 className="text-xl font-semibold text-gray-700 mb-4 text-center">Add New Product</h2>
